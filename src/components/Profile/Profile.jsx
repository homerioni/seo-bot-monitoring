...
const Profile = ({profileData, setModalUser}) => {
    const isAdmin = !!profileData.result.roles.find(role => role.id === 1);
        ...
        <div className={s.main}>
            <div className={s.header}>
                ...
                <div className={s.nameBox}>
                    <p className={s.name}>{profileData?.result.firstName} {profileData.result.lastName}</p>
                    <p className={s.role}><b>Роли:</b> {profileData?.result.roles.map(role => role.alias).join(', ')}.</p>
                </div>
            </div>
            {isAdmin && <ProfileList setModalUser={setModalUser}/>}
        </div>
    );
};
