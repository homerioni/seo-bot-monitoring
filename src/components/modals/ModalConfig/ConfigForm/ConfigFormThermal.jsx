...
const ConfigFormThermal = ({register, handleSubmit, isValid, setClose, submitFunc}) => {
    const [isLoading, setIsLoading] = useState(false);

    const onSubmit = reqData =>
        submitFunc(reqData, setIsLoading, 'THERMAL_PASTE', `Термопаста ${reqData.name} успешно сохранена`);

    return (
        <form className={s.form} onSubmit={handleSubmit(onSubmit)}>
            <div className={s.formContent}>
                <InputText register={{...register('name', {required: true})}} className={s.auto} title='Название'/>
                <InputText register={{...register('characteristics.capacity', {required: true})}} className={s.flex10} title='Объем, г.'/>
...
export default ConfigFormThermal;