...
const SubmitBtn = ({type, isValid, isLoading, text, isSaved, className, onClick}) => {
    return (
        <button type={type ?? "submit"} className={`${s.submit} ${!isValid ? s.disable : ''} ${isSaved ? s.active : ''} ${className}`} onClick={onClick}>
            {isLoading ? <Loading/> : ''}
            ...
            <span>{isSaved ? 'Сохранено' : (text ?? 'Сохранить')}</span>
        </button>
    );
};
