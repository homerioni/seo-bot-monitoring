...
import s from './SelectTextList.module.scss';
import {useClosingDropDownList} from "../../../../hooks/useClosingDropDownList";
import TitleLabel from "../TitleLabel/TitleLabel";
import Loading from "../../Loading/Loading";

const SelectTextList = ({selectedWatch, list, register, className, title, tooltip, placeholder, error, isLoading}) => {
    ...
    useEffect(() => {
        setSelected(list?.find(el => el.val == selectedWatch));
    }, [selectedWatch]);

    const onInput = () => setIsListOpen(false);

    const listRender = filteredList?.map((item, i) => {
        return (
            <label key={item.val}>
                <input {...register} type="radio" value={item.val} onInput={onInput}/>
                <span>{i + 1}. </span>
                <b>{item.name}</b>
            </label>
        );
    ...
    return (
        <div className={`${className ?? s.main} ${isListOpen ? s.upper : ''}`}>
            ...
            <div className={s.labelBox}>
                <div className={`${s.label} ${error ? s.error : ''}`} onClick={() => setIsListOpen(!isListOpen)}>
                    <p>{selected ? selected.name : <span className={s.gray}>{placeholder}</span>}</p>
